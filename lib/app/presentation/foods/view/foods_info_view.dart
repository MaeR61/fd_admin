...
class FoodsInfoView extends StatefulWidget {
  const FoodsInfoView({super.key});

  @override
  State<FoodsInfoView> createState() => _FoodsInfoViewState();
}

class _FoodsInfoViewState extends State<FoodsInfoView> {
  @override
  Widget build(BuildContext context) {
    final dark = MaeRHelperFunctions(context).isDarkMode;
    final foods = context.watch<FoodsInfoViewModel>().state.foods;
                ...
                ? Center(
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (_, index) => foodsInfoCard(
                          context,
                          foods.data![index],
                          index,
                          dark,
  ...
  MaeRHorizontalCard foodsInfoCard(
      ...
      child: Column(
        ...
        children: [
          ...
          ListView.builder(
            shrinkWrap: true,
            physics: NeverScrollableScrollPhysics(),
            itemCount: food.outcomes.length,
          ...
          Text.rich(
            TextSpan(
              ...
              children: [
                TextSpan(
                  text: MaeRFormatter.formatDateWithSpace(food.dateStarted),
      ...
      child: Column(
        children: [
          ...
          GestureDetector(
            ...
            onTap: () async {
              final selected = await showModalBottomSheet<FoodSortOrder>(
                context: context,
                builder: (_) => _SortBottomSheet(
                  selectedOrder: context
                      .read<FoodsInfoViewModel>()
                      .state
                      .sortOrder,
                ),
              );
              if (selected != null) {
                if (context.mounted) {
                  context.read<FoodsInfoViewModel>().updateSortOrder(selected);
                }
              }
            },
            child: Container(
          ...
          GestureDetector(
            behavior: HitTestBehavior.translucent,
            onTap: () {
              context.read<FoodsInfoViewModel>().fetchFoods();
            },
            child: Container(
              width: context.dynamicWidth(0.7),
              height: context.dynamicHeight(0.05),
    ...
    return Row(
      ...
      children: [
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () {
            context.read<FoodsInfoViewModel>().decreaseQuarter();
          },
          child: Icon(
            Icons.arrow_left,
            size: 32,
        ...
        Text(
          context.watch<FoodsInfoViewModel>().state.quarter.displayName,
        ...
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () {
            context.read<FoodsInfoViewModel>().increaseQuarter();
          },
          child: Icon(
            Icons.arrow_right,
            size: 32,
  ...
  Row _yearPicker(BuildContext context) {
    return Row(
      ...
      children: [
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () => context.read<FoodsInfoViewModel>().decreaseYear(),
          child: Icon(
            Icons.arrow_left,
            size: 32,
            color: context.watch<FoodsInfoViewModel>().state.year != 0
                ? ColorConstants.colorPrimary
                : ColorConstants.colorGray,
          ),
        ),
        Text(
          context.watch<FoodsInfoViewModel>().state.year.toString(),
          style: FontStyles.w6s16.copyWith(
            color: dark ? ColorConstants.colorWhite : ColorConstants.colorBlack,
          ),
        ),
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () => context.read<FoodsInfoViewModel>().increaseYear(),
          child: Icon(
            Icons.arrow_right,
            size: 32,
            color:
                context.watch<FoodsInfoViewModel>().state.year !=
                    DateTime.now().year
                ? ColorConstants.colorPrimary
                : ColorConstants.colorGray,
          ),
...
class _SortBottomSheet extends StatelessWidget {
  final FoodSortOrder selectedOrder;
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: FoodSortOrder.values.map((order) {
