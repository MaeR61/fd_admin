...
class TobaccoInfoView extends StatefulWidget {
  const TobaccoInfoView({super.key});

  @override
  State<TobaccoInfoView> createState() => _TobaccoInfoViewState();
}

class _TobaccoInfoViewState extends State<TobaccoInfoView> {
  @override
  Widget build(BuildContext context) {
    final dark = MaeRHelperFunctions(context).isDarkMode;
    final tobacco = context.watch<TobaccoInfoViewModel>().state.tobacco;
                ...
                ? Center(
                    child: SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemCount: tobacco.data?.length ?? 0,
                        itemBuilder: (_, index) => tobaccoInfoCard(
                          context,
                          tobacco.data![index],
                          index,
                          dark,
  ...
  MaeRHorizontalCard tobaccoInfoCard(
    BuildContext context,
    TobaccoModel tobacco,
    int index,
    bool dark,
  ) {
    return MaeRHorizontalCard(
      ...
      expansionChild: Column(
        children: List.generate(
          tobacco.tobaccoProducts?.length ?? 0,
          (i) => Text(tobacco.tobaccoProducts?[i] ?? ""),
        ),
      ),
      child: Column(
        ...
        children: [
          ...
          ListView.builder(
            shrinkWrap: true,
            physics: NeverScrollableScrollPhysics(),
            itemCount: tobacco.reportedHealthProblems?.length ?? 0,
            itemBuilder: (context, i) => Text(
              tobacco.reportedHealthProblems?[i] ?? "",
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),
                  itemCount: tobacco.reportedProductProblems?.length ?? 0,
                  itemBuilder: (context, i) => Text(
                    tobacco.reportedProductProblems?[i] ?? "",
                    style: FontStyles.w5s14.copyWith(
                      color: dark
                          ? ColorConstants.colorWhite
                          : ColorConstants.colorBlack,
      ...
      child: Column(
        children: [
          ...
          GestureDetector(
            ...
            onTap: () async {
              final selected = await showModalBottomSheet<TobaccoSortOrder>(
                context: context,
                builder: (_) => _SortBottomSheet(
                  selectedOrder: context
                      .read<TobaccoInfoViewModel>()
                      .state
                      .sortOrder,
                ),
              );
              if (selected != null) {
                if (context.mounted) {
                  context.read<TobaccoInfoViewModel>().updateSortOrder(
                    selected,
                  );
                }
              }
          ...
          GestureDetector(
            behavior: HitTestBehavior.translucent,
            onTap: () {
              context.read<TobaccoInfoViewModel>().fetchTobaccos();
            },
            child: Container(
              width: context.dynamicWidth(0.7),
              height: context.dynamicHeight(0.05),
    ...
    return Row(
      ...
      children: [
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () {
            context.read<TobaccoInfoViewModel>().decreaseQuarter();
          },
          child: Icon(
            Icons.arrow_left,
            size: 32,
        ...
        Text(
          context.watch<TobaccoInfoViewModel>().state.quarter.displayName,
        ...
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () {
            context.read<TobaccoInfoViewModel>().increaseQuarter();
          },
          child: Icon(
            Icons.arrow_right,
            size: 32,
  ...
  Row _yearPicker(BuildContext context) {
    return Row(
      ...
      children: [
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () => context.read<TobaccoInfoViewModel>().decreaseYear(),
          child: Icon(
            Icons.arrow_left,
            size: 32,
            color: context.watch<TobaccoInfoViewModel>().state.year != 0
                ? ColorConstants.colorPrimary
                : ColorConstants.colorGray,
          ),
        ),
        Text(
          context.watch<TobaccoInfoViewModel>().state.year.toString(),
          style: FontStyles.w6s16.copyWith(
            color: dark ? ColorConstants.colorWhite : ColorConstants.colorBlack,
          ),
        ),
        GestureDetector(
          behavior: HitTestBehavior.translucent,
          onTap: () => context.read<TobaccoInfoViewModel>().increaseYear(),
          child: Icon(
            Icons.arrow_right,
            size: 32,
            color:
                context.watch<TobaccoInfoViewModel>().state.year !=
                    DateTime.now().year
                ? ColorConstants.colorPrimary
                : ColorConstants.colorGray,
          ),
...
class _SortBottomSheet extends StatelessWidget {
  final TobaccoSortOrder selectedOrder;
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Column(
        ...
        children: TobaccoSortOrder.values.map((order) {
